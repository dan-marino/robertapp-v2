...
/**
 * Fetch all game context, run the Lineup Generator, persist the result
 * (replacing any existing slots), and return the new Lineup.
 *
 * Used by both the Generate and Reshuffle endpoints.
 */
export async function generateLineupForGame(gameId: string) {
  ...
  if (activeRoster.length === 0) return 'no-active-players' as const

  const latePlayerIds = getLatePlayers(allPlayers, rsvpList).map((p) => p.id)
  ...
  const posHistRows = activePids.length > 0
    ? await db.select().from(positionHistoryTable).where(
        and(
  ...
  const batHistRows = activePids.length > 0
    ? await db.select().from(battingHistoryTable).where(
        and(
